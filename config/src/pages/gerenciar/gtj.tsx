...
export function GTJ() {
  ...
  useEffect(() => {
    const fetchAlunos = async () => {
      try {
        const response = await axios.get<Aluno[]>(`${import.meta.env.VITE_SERVIDOR_URL}/alunos/search/juniores`, {
          params: { sexo: filtroSexo, ordem: filtroOrdem }
        });
        setAlunos(response.data);
        setLoading(false);
      } catch (error) {
        console.error('Erro ao buscar alunos do juniores:', error);
        setLoading(false);
        setAlerta({ status: 'error', mensagem: 'Erro ao buscar alunos do juniores.' });
      }
    };
  ...
  return (
    <>
      <Helmet>
        <title>Turma do Juniores • Sistema Kids | Ministério Kids</title>
        <meta name="title" content="Turma do Juniores • Sistema Kids | Ministério Kids" />
      </Helmet>
      <section className="_body _turma">
        ...
        <main className="_main">
          ...
          <section className="_card">
            <h2 className="center">Turma do Juniores</h2>
            <div className="_div">
              {loading ? (
                <Loading />
              ) : (
