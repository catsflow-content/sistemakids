...
export function GCM() {
  ...
  useEffect(() => {
    const fetchChamadas = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_SERVIDOR_URL}/chamadas/gerenciar/maternal?ordem=${ordem}&professor=${professor}`);
  ...
  const confirmDelete = async () => {
    if (chamadaIdToDelete) {
      try {
        const response = await fetch(`${import.meta.env.VITE_SERVIDOR_URL}/chamadas/excluir/maternal/${chamadaIdToDelete}`, {
          method: 'DELETE',
        });
  ...
  const handleVisualizarChamada = (id: string) => {
    sessionStorage.setItem('CMID', id);
  ...
  return (
    <section className="_body _turma">
      ...
      <main className="_main">
        ...
        <section className="_card">
          <h2 className="center">Chamadas do maternal</h2>
          <div className="_div">
            {loading ? (
              <Loading />
