...
export function Maternal() {
  ...
  useEffect(() => {
    async function fetchAlunosMaternal() {
      try {
        const response = await fetch(`${import.meta.env.VITE_SERVIDOR_URL}/alunos/search/maternal`);
        if (!response.ok) throw new Error('Erro ao buscar alunos do maternal');
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SERVIDOR_URL}/chamadas/register/maternal`, {
        method: 'POST',
        headers: {
      ...
      const lastChamadaResponse = await fetch(`${import.meta.env.VITE_SERVIDOR_URL}/chamadas/last/maternal`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      sessionStorage.setItem('CMID', lastChamadaData.id.toString());
      setAlertas([{ status: 'success', mensagem: 'Aula cadastrada com sucesso' }]);

      window.location.href = '/turma/maternal/chamada/view';
    } catch (error) {
  ...
  return (
    <section className="_body _turma">
      <Helmet>
        <title>Registrar chamada dos maternal • Sistema Kids | Ministério Kids</title>
        <meta name="title" content="Registrar chamada dos maternal • Sistema Kids | Ministério Kids" />
      </Helmet>
      <Header />
      <form onSubmit={handleSubmit} className="_main">
                ...
                <label className="_select" htmlFor="selprofessor">
                  <Select className="camp" id="selprofessor" placeholder="Selecionar professor" name="Professor" title="Nome do professor" value={selectedProfessor} onChange={handleProfessorChange} required>
                    ...
                    <optgroup label="Professores">
                      {usuarios.filter(usuario => usuario.permission === 'maternal').map(usuario => (
                        <option key={usuario.id} value={usuario.nome}>{usuario.nome}</option>
                      ))}
                    </optgroup>
                  </Select>
        ...
        <section className="_card">
          <h2 className="center">Alunos do maternal</h2>
          {loading ? (
            <Loading />
          ) : (
            <>
